...
- `pitcher_score(r)` → 0–100. Role-aware via `_is_sp(r)`. **SP path**: role bonus 9–12 based on GS volume; SVHD ignored. **RP path**: role bonus 5–12 scaled by SVHD + W (up to 6pts) + IP/G opportunity (up to 5pts). K component uses derived K% (`Kpct_P`), else K/IP; ERA component uses ERA. (WhiffPct and xFIP aren't in the pipeline — FanGraphs 403 — so those branches were removed; scores are unchanged since both fields were always absent.) **Small-sample penalty**: `s *= min(1.0, ip / 20)` applied before calibration — suppresses sub-20-IP samples. Calibrated to p50=50, p90=80: `s * 1.875 - 67.6`.
