...
**Compacted 9-column tables (My Upcoming Starts + FA Starting Pitchers):** These two are the widest tables and overflow an iPad width (they sit in `overflow-x:auto` wrappers). Each builds **local** tight style vars — `_th`/`_tdc`/`_tds` = `TH_S`/`TDC`/`TD_S` with horizontal padding `10px → 6px` and body font `13px → 12px` (via `.replace()` so they track the shared constants) — and the table's own inline `font-size` is `12px`. The swap is **scoped to these two blocks only**; the shared `TH_S`/`TDC`/`TD_S` constants (used by every other table) are untouched, so narrower tables keep the roomier look. `hot_cold_cell(..., td_style=_tdc)` takes an optional `td_style` (defaults to `TDC`) so the L15 ERA column matches the compacted cells. Keep the two tables' padding/font in sync — they're meant to look identical. The `colspan="9"` date-header banner rows keep their own inline padding (full-width, don't set column widths).
