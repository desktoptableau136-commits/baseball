...
- **Win-probability (`_cat_win_prob` + `compute_weekly_std`):** each card's top-right corner shows a `WIN %` chip (green ≥ 65, red ≤ 35, else yellow), and the summary adds an `expected finish N W · N L · N T` line — whole numbers (Σ p_win / Σ p_tie / remainder, then `_largest_remainder_round` so they sum exactly to the category count) with the exact fractional in the hover `title`. `compute_weekly_std(roto, week)` mirrors `compute_weekly_avgs` (same buckets) → per-team/per-cat population stddev, needs ≥ 2 completed weeks; threaded into `build_category_pulse(weekly_std=…)` from `build_email`. `_cat_win_prob(pm, po, cat, sigma, remaining_frac)` returns `(p_win, p_tie)` from a normal-CDF model (`math.erf`, no scipy) of the final margin: `edge` is direction-adjusted for `_LOWER_BETTER`; `sigma` = combined `sqrt(my_std² + opp_std²)` (falls back to `_CLOSE_THRESH[cat]` when either team has no history); counting-cat uncertainty scales × `remaining_frac` (rate cats keep their weekly spread). **The tie band half-width is `0.5·10^-dec` (a display unit), matching the point-estimate `round(pm,dec)==round(po,dec)` tie test — so the WIN % can never contradict the proj W/L/T direction** (verified: 0 mismatches across 12 cards). Display-only enrichment — `classify_categories` is untouched, so the FA Cats need-set and pickup steering are unaffected.
...
**FA SP / My Upcoming Starts columns (8):** Pitcher · Proj. Line · Matchup · QS% · ERA · L15 ERA · K% · Score. "Proj. Line" = projected `IP · ER · K` per start, IP in baseball notation via `_fmt_ip()`. **Opp OPS is folded into the Matchup cell** as a muted second line (`_opp_ops_sub(r)` → "Opp OPS .742"). Date-header rows span `colspan="8"` with background on `<tr>`.
