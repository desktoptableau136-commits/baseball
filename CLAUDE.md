...
`week_end_str` comes from `matchup_end_date` in the snapshot (not a hardcoded `today + 6 - weekday`). `fetch_data.get_matchup_dates(league)` reads `league.settings.matchup_periods` — a dict mapping matchup period → list of **weekly** scoring-period IDs (e.g. `{'15': [15]}` for a 7-day week, `{'16': [16, 17]}` for a 14-day All-Star period). Period length = `len(ids) * 7 days`. Stored in snapshot as `matchup_start_date`, `matchup_end_date`, `matchup_period_days`, `next_matchup_end_date`. Falls back to calendar-week Sunday when snapshot predates this field. FA SP + My Upcoming Starts show all starts including next period, but dates past `week_end_str` get a `NEXT WK` badge. The "Starts This Week" KPI and Week-at-a-Glance bullets 2–3 count/recommend only within the current matchup period. **Bullet 2 scopes both `confirmed` and `thin_days` to `PSP_Date <= week_end`** so its count matches the KPI.
