...
**Tap-to-expand score breakdown v2 (`score_reveal` / `_hitter_score_breakdown` / `_pitcher_score_breakdown` / `_score_narrative`):** Tapping a Score badge reveals a **full-width row rendered directly below the player's row** (not a panel inside the narrow Score cell — that's the v1 look the user found cramped). The panel narrates the score's **2–3 most decisive drivers in prose** — e.g. `SP score 63. Carried by swing-and-miss (24% K) and limits baserunners (1.23 WHIP); no glaring holes. Recent form 58 (cold) → shown blends 65% season / 35% recent.` — not a `points/max` list.
...
- **Wired into all Score badges:** the 7 tables (Roster Hot/Cold, Pitcher Hot/Cold, My Upcoming Starts, My Relief Pitchers, FA SP, FA RP, FA Hitters) **plus Positional Breakdown** (both the weakest-my-player badge `posw` and the best-FA badge `posfa`, role-aware via `p["ptype"]`, `colspan=4`). No bare `badge()` renders a displayed score anymore — only `two_start_badge()` and `score_reveal`'s own fallback remain. **HR%'s `_hrp_cell` is still hover-only** (`title=`) — convert the same way if mobile HR% drivers are wanted. The glossary section (`build_glossary_section`) keeps its own `<details>` groups (unrelated).
