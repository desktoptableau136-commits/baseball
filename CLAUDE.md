...
**Tap-to-expand score breakdown (`score_reveal` / `_hitter_score_breakdown` / `_pitcher_score_breakdown`):** Every Score badge is wrapped in a `<details class="scorebd">` whose `<summary>` is the `badge()` and whose expanded panel is the plain-language component breakdown (e.g. `Season 61 — Prod 16/30 · HR 3/16 · … · HR% 6/8`, then the season↔recent blend line). **Why `<details>` and not a `title=` hover tooltip:** the user reads on phone/tablet, where hover doesn't exist and email clients strip the JS/CSS a custom tooltip needs; native `<details>` is the only no-JS tap mechanism, and it works in the browser-rendered **attachment** (mobile Safari/Chrome). It degrades to always-visible (badge + text) where a client ignores the toggle — never blank. The breakdown builders call `hitter_score`/`pitcher_score`/`rp_score` with `_parts=True` so they can't drift from the real math (see the scoring-functions section). `_pitcher_score_breakdown` is role-aware (SP → `pitcher_score` comps + blend note; RP → `rp_score` comps, no blend). The ▸/▾ caret + marker-hiding is a `<style>` rule in the head block (`details.scorebd summary::after` / `::-webkit-details-marker`) — lives in the attachment's `<style>`, stripped by Gmail inline (harmless). Wired into all 7 Score-badge tables: Roster Hot/Cold, Pitcher Hot/Cold, My Upcoming Starts, My Relief Pitchers, FA SP, FA RP, FA Hitters (the two Hot/Cold sections stash the season row as `srow` for the panel; `best_recent_p`/`best_recent_h` are closured from `build_email`). **HR%'s `_hrp_cell` is still hover-only** (`title=`) — not yet converted to tap; convert it the same way if mobile HR% drivers are wanted. The glossary section (`build_glossary_section`) already used `<details>` for its own collapsible groups (no `scorebd` class — its marker styling is inline).
