...
### Roster Hot/Cold
Your rostered **hitters** sorted hottest → coldest. Compares last-7-day OPS to season OPS. Includes an **HR%** column (modeled per-game home-run probability, hover for drivers — also listed inside the expanded Score panel for touch devices) and a **Score** badge. Tapping the Score badge shows a breakdown that opens with the **season and recent scores** (each color-coded, with a 🔥/🥶 form marker), a compact **Last 30 days** line naming the window stats behind the recent number (e.g. "1.028 OPS · .275 AVG · 8 HR"), and a one-line **scouting read** of the player's archetype, then the drivers behind the number. Its recent window (e.g. 30-day) is broader than this L7 column, so a bat can be 🔥 here yet read "cold" there.
