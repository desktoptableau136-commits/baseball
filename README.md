...
4b. **Today's MLB Games** — real games today ranked by how much they overlap your matchup (weighted toward your roster; counts hitters + confirmed starters), with first-pitch time, where to watch (national + local TV), the pitching matchup, and per-side player counts *(what to tune into; renders only when a game overlaps your matchup)*
