...
**TRANSACTIONS**
12a. **Weekly Game Plan** — a 🎯 Win-the-week odds header, a Locked/Contest/Concede category triage, and up to 3 ranked FA move cards *(see [Weekly Game Plan](#weekly-game-plan) below; suppressed on the final day of a matchup, though the odds still show)*
...
A small diagonal arrow (**$↗** / **▼↘**) means that call isn't just a season-level prediction — it's already showing up in his recent games (his recent AVG/SLG, weighted for small sample size, still diverges from his season expected stats in the same direction). No arrow doesn't mean the call is wrong, just that there's no recent confirmation yet either way; it's meant to be rare, a stronger version of the same $/▼ signal rather than a separate one. If his recent games are trending the *opposite* way instead, the badge stays plain but the hover text flags the contradiction.

A **bare arrow with no $/▼** (just 🟢**↗** / 🔴**↘**) means his season totals haven't moved enough yet to trip the full buy-low/sell-high badge, but his recent games are already trending that direction against his own expected stats — an early read that can graduate to a full $/▼ once the season numbers catch up.
...
**Up to 3 ranked moves.** Candidates are drawn *only* from the same FA Starting Pitcher / Relief Pitcher / Hitter pools shown below — the plan can never recommend a player those tables don't also list — ranked by how much each one lifts your **overall win-the-week odds**, not just a single category. Each card shows the add (with his projected line and start day for a pitcher, or his season contribution for a hitter/reliever), the drop it costs (or a free roster spot), and the before→after odds for both the category and the whole week. A right-aligned tag reads **hold** when the add is a durable season-quality upgrade over your starter at that spot, or **streamer** for a short-term volume/form play.
