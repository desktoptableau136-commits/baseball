...
3. **`weekly_recap.py`** reads the same snapshot every Monday and emails a full-league recap (1100px-wide layout): **Matchup N Highlights** (commissioner-style prose + stat sidebar — roto winner, hitter-of-the-matchup, **K-first** pitcher-of-the-matchup, a separate **best available bat** and **best available arm** free-agent callout, and a league-wide **Matchup of the Week** drama pick, with MLB team logos and named historical benchmarks), your matchup result, **Lineup Efficiency** (last matchup's start/sit opportunity cost — bench leakage + good starts left on the bench + active-slot pitcher blowups, laid out two columns wide), all 6 scoreboard matchups (two per row), Matchup Roto Rankings (all 12 categories, 5-tier heat-map coloring), Top Performers (hitters and **K-first** pitchers side-by-side), Standings & Luck, Season Trajectory, and Season Roto Rankings (the same 12-category grid aggregated over every matchup — ranked by cumulative roto points, each category showing its true season-to-date value from ESPN). Saves `previews/recap_week_N.html` on dry runs. GitHub Actions: `.github/workflows/weekly-recap.yml` (Monday 15:30 UTC).
...
A glance-able "command dashboard" that condenses the entire digest onto **one 1440×900 laptop screen with zero scrolling** — even coverage of every topic (today's games, category pulse, pitching, hitter recent form, weakest spots, moves, free agents, trade radar, season). It reuses the digest's exact scoring so every number matches.
...
The **My Pitching** tile lists each upcoming start with its projected `IP·ER·K` line, a blue `×2` marker for two-start pitchers, cyan **QS** / yellow **5K+** badges, an orange **⚠** low-floor (blowup-risk) chip, and the **$ / ▼ / ▽** buy-low / sell-high chip after the matchup date (same projected-line, risk, and regression rules as the digest — sell-high renders hollow **▽** until confirmed by his recent games, solid **▼** after). The **Free-Agent Radar** (starters and relievers) and **Weakest Spots** (pitcher rows) carry the ⚠ and $ / ▼ / ▽ chips too. A compact **Trade Radar** tile (in place of a standalone opponent tile — opponent scouting lives in the digest) shows the top couple of mutually-beneficial trade suggestions (two distinct partners), each laid out as a 3-column mini-card — partner + value tilt on the left, then **Give** and **Get** columns of players — with a **score pill** (hover for a short breakdown), plus the same tactical skill badges as everywhere else (PWR/SB for hitters, cyan **QS** / yellow **K+** season-skill + orange **⚠** for starters) and the $ / ▼ / ▽ and position chips — the full list lives in the daily digest. **When you have a real incoming trade offer, it leads this tile** (retitled **"Trades"**) as an "📥 Offer to review" card with the **Accept / Counter / Decline** verdict, days-until-expiry, and the counter suggestion — a concrete decision on the clock outranks the speculative ideas, which drop to one to keep the layout scroll-free. The Trade Radar tile header also carries a **Trade Lab ↗** link that opens the hosted pocket Trade Lab (the same builder the digest's trade cards deep-link into), so you can jump straight from an idea to constructing your own deal. The **Recommended Moves** tile shows a **score pill** next to every add and drop player, so you can gauge the quality of a suggested pickup and the player it costs at a glance. **MLB team logos** appear next to player names throughout the dashboard (Today's Games, My Pitching, Hitter Recent Form, Weakest Spots, Free-Agent Radar, Trade Radar). The browser tab title reads **"Dashboard — {team}"** (and the daily digest reads **"Daily Digest — {team}"**) so the type is identifiable at a glance. A **legend** defines every badge/marker at a glance (score pill, ▲▼◆ projected outcome, ⚡ toss-up, ×2, QS, 5K+, ⚠, $ / ▼ / ▽, PWR, SB, 🔥/❄) — a slim strip pinned to the bottom of the pane on desktop, and a "Key" panel at the bottom of the right column (below Trade Radar) on tablet/phone.

It's also **responsive**: on a tablet (≤1100px) the tiles reflow into two height-balanced columns — Today's Games → Category Pulse → Recommended Moves → Free-Agent Radar down the left, then My Pitching → Hitter Recent Form → Weakest Spots → Trade Radar → Season down the right — and on a phone (≤700px) into a single column, un-pinning the fixed pane so the page scrolls normally with larger, readable text — with the top KPI chips (Record through Luck) shrinking just enough to stay on a single row rather than wrapping on a narrow phone. The desktop no-scroll layout is unchanged above 1100px.
...
At the top of the page, a **"Who should you be trading with?" board** answers the question *before* you pick anyone. It ranks every rival by how landable a deal actually is and shows **one concrete, engine-graded offer each, in plain English** — e.g. *"Get Otto Lopez + Jeff Hoffman for Jake McCarthy + OF depth — realistic, fills a need."* Every player in the deal carries a small **position chip** — muted by default, lit up **lime** when that slot is one of your thin positions, so you can tell at a glance whether the incoming piece actually plugs a hole without reading the prose. Cleaner deals sort to the top as **BEST TARGET** (realistic, lands a need — the rival clearly gains too) or **WORTH A SHOT** (still a realistic ask the rival wouldn't lose on, they just don't gain much — less compelling than a BEST, but not a stretch), each carrying the same **estimated likelihood %** the digest's Trade Radar shows; an ask that would require prying a rival's genuine star at par is never shown as a target at all — it drops straight to a **dead end** instead. The dead ends sink to the bottom *with their reason* — **SLIM** (overlap exists but no realistic, near-even deal came together — including when the only shape available was a star-reach), **ONE-WAY** (they'd help you, but you can't fill a hole of theirs, so you'd overpay), and **NO DEAL** (category twins — you share the same strengths and the same holes, so there's nothing to arbitrage). A **SLIM** card isn't always a total dead end, though: when a real offer exists but stalls short of a clean recommendation, the card still shows that exact deal (dimmed, no green verdict) with its own **Build this ▶** button and a one-line reason for why it's not quite there — either it'd mean prying loose a rival's genuine star (*"real value edge for you, but Bobby Witt Jr. is a bona fide star — they'd likely hold at this price"*), or it's a real idea on your side that just needs a bit more to close (*"right direction but you'd be paying up — ask for more"*) — so you can judge it yourself instead of just being told to look manually. This is especially useful when your needs sit at scarce, single-roster-slot positions like catcher or shortstop, where almost every rival is thin too — the board now digs harder to find the best available idea instead of coming up empty. Each live card has a **Build this ▶** button that drops the deal straight into the builder below. Above the per-rival board, a **💫 Blockbuster deals** strip surfaces the top couple of **multi-player consolidation win-wins** (give roster depth, get fewer-but-better) — the hand-built kind of deal the ≤2-per-side board can't form — one per partner so it mixes up teams, each with a one-line read on why it's exciting and the same **Build this ▶** one-click load. The board is graded by the same trade engine as the digest's Trade Radar, and it re-computes for whichever team you put on the left, so you can scout any team's trade landscape.
...
A third marker — a **red ↓ "drop candidate"** — answers a different question entirely: not "does he fit a trade?" but "is there a better body sitting on waivers right now?" It compares a player's value against the actual best free agent available at his position/role, so it's grounded in what you could add today, not a guess. On **your own roster**, it overrides the amber arrow — a too-cheap player of yours isn't a real trade chip no matter what the need math says, so trading him is a waste of time; cut him and stream the better free agent instead. On the **partner's roster**, it never hides a green grab arrow — "he fills your need" stays true whether or not his own team could easily replace him, and in fact tells you something useful: it's a sign to check the wire before you give anything up for him, or to lean on his low value as leverage while bundling him into a bigger offer.

The center **Trade** column stays calm on purpose: a verdict, the give/get lists with a fairness bar and a plain-English takeaway, and a one-line read of how each side feels about it. The supporting numbers live one tap away — **tap any player** to see why he's worth what he is to each side (his categories and position, then how your roster and theirs re-price him), or open **Value detail** for the full give/get/net breakdown, with a plain-English caption explaining what the "value" number actually means.
...
Open the file in a browser (it's the only page here that uses JavaScript, so it **can't be emailed** — mail clients strip the script). The layout is three columns — **My Team · Trade · Trade Partner**. Each side has a team dropdown (any of the 12 teams) and lists that roster as **Hitters / Starting Pitchers / Relief Pitchers**, every player carrying the same tactical badges (PWR, SB, ⚠, $ / ▼ / ▽) and a color-coded **score pill** you can click to expand a plain-English breakdown — identical to the digest. Starters additionally carry two **season skill** badges — a cyan **QS** for a reliable quality-start arm and a yellow **K+** for an elite-strikeout arm — the season-long counterpart to the digest's per-start QS / 5K+ chips (so you can size up a starter's durable value at a glance when weighing a deal).

Click players to drop them into the center **You give / You get** ledger and the panel grades the deal live: a top **odds pill**, a **three-row value block** — **Base** (universal value), **My value**, and **Their value** — each showing give / get / net, so you can see when a deal that looks lopsided on paper is actually win-win because each side values what it *receives* more (a piece is worth more to a team that needs the position/category than to one already set there). Below that: the categories you gain (green when they fill one of your needs) and lose (red when it's a real strength), any positional upgrades, and a two-sided acceptance read — each side now shows its own **Accept / Counter / Decline** pill plus that side's own **likelihood %**, so "how would each manager actually feel about this" reads at a glance instead of a bare thumbs-up/down. **"Would they do it?"** is deliberately skeptical: the rival has to come out at least even *by their own needs*, and prying a genuine star reads as an aggressive ask unless you overpay — the better their player, the bigger the overpay expected (a real scarce-position pickup — a genuine catcher or shortstop — gets a small, documented discount on that skepticism, since you've already signaled you'll pay a premium there). **"Would you do it?"** does the mirror for your side (flagging when *you'd* be surrendering your star too cheaply). A star-at-par pry on either side also nudges its pill toward Counter. The **top pill answers a different question — will this whole deal actually go through** — and reads **Good Chance / Worth a Shot / Likely Rejected** off a single combined percentage that leans toward the rival's read (you're the one deciding which deals even get proposed, so their willingness matters most once it's built) without echoing either side's own Accept/Counter/Decline label, so it can't be mistaken for a one-sided verdict. Every number is pre-computed by the same scoring code as the digest, so the lab can never disagree with it.

Two things help you build the deal quickly. Each player carries a **position chip** by their name, and partner players who'd actually help you are flagged with a **🎯 target** marker (they're strong in a category you need, or upgrade one of your thin positions). Below the ledger, a **Deal Coach** panel shows what each side still needs — split into **category needs** and **position needs**, so a stat gap and a roster-slot gap never read as the same thing — plus your leverage (categories *and* positions where you're deep and they're thin). It then lists **clickable suggestions**, ranked not just by raw trade value but by how much a player actually helps the need side **and** how realistic it is that his own team would actually part with him — a rival's genuinely spare depth piece now outranks a star of theirs who fills the same need, so the Coach doesn't just default to naming their best player every time. Any candidate who'd leave a roster below startable depth at a position is excluded outright, never just flagged — a pick that clears that bar still shows a warning glyph if it'd leave a position at the bare minimum with no backup, or a star-value flag if it's the kind of piece that usually costs a sweetener, plus a quick **estimated accept-odds %**. When nothing squarely matches a need, the list pads in a few clearly-marked **pure-value** picks (dashed, `≈`) rather than pretending they're a recommendation. A **Fair · Favor me · Fleece** strategy toggle tunes how the coach curates on BOTH sides: sliding toward *Fleece* protects your best players from the offer list, leads with cheaper fillers on your side, and lets the suggestions reach further for the partner's best matching player instead of only their realistically-available depth; *Fair* leans the opposite way, leading with the safest, most landable ask on both sides. The toggle only shapes the coaching — the Accept/Counter/Decline verdict stays objective. When a deal grades **Counter** because you'd be paying up, the Coach panel itself **names the specific spare partner piece to request** (a clickable name that adds it straight to the *You get* side, never a star or a piece that would leave the partner short at a position already in the deal), mirroring the digest's counter suggestion — and flags it right alongside a note when your build is one piece away from blockbuster territory.
...
3. **Category Pulse** — summary line ends with a **🏆 Win the week** chip, the odds you take more categories than your opponent this matchup
...
11. **Pitcher Recent Form**
12. **Hitter Recent Form**

**TRANSACTIONS**
12a. **Weekly Game Plan** — a 🏆 Win-the-week odds header, a Locked/Contest/Concede category triage, and up to 4 ranked FA move cards (2 hitter + 2 pitcher, side by side in two columns) *(see [Weekly Game Plan](#weekly-game-plan) below; suppressed on the final day of a matchup, though the odds still show)*
...
15b. **Trade Radar** — mutually-beneficial trade ideas with rival teams, each with a quiet **base/you/them net-value** hint and a **Build in Trade Lab** link *(shown only when candidates exist)*. When one exists, a purple **💫 Blockbuster** card headlines the list: a multi-player **consolidation** win-win (give several depth pieces, get fewer-but-better need-fillers) that the standard ≤2-per-side deals can't form — kept to genuine two-sided wins only.
...
4. **Pickups (roster-context aware)** — up to two targeted add/drop bullets:
   ...
   Drops prefer a **surplus** player (a deep position or a bench-leaker), tagged `[surplus]`, and the two bullets never suggest dropping the same player. If you have an open **active** roster spot the add shows as a free pickup instead (your league's real active/IL slot split, pulled from ESPN — not a guessed number). Drops never target an injured player sitting in one of your **2 IL roster slots** (cutting them frees nothing), and always leave ≥ 1 healthy player at every position **after the add** — e.g. dropping your only shortstop is fine when the recommended add is himself SS-eligible, since the position isn't actually left empty.
...
Each of the top few games shows both teams' logos, first-pitch time (ET), **where to watch** (the national network when there is one, plus each side's local TV feed so you always have a channel), the **pitching matchup** (`⚾ SP: away vs home`, with a probable highlighted in blue if he's yours / red if your opponent's), and a **"You: N / Opp: N"** line naming the involved players (a ⚾ marks a confirmed starter) — each with the **same tactical badges** the rest of the digest uses (power/speed/buy/sell for hitters, blowup-risk/buy/sell for pitchers). Your **favorite team's games (Atlanta) are pinned to the top** (marked ★) regardless of overlap score. A real **doubleheader** collapses to a single card (both legs share the same roster overlap, so they'd otherwise render as duplicates) — the kept card gets a small **(DH)** note so a second game isn't silently hidden. The single highest-overlap game also gets a one-line **📺 Tune in** teaser (with its network) at the top of the email body. Hitters are counted optimistically — we don't have posted MLB batting lineups, so only starting pitchers are confirmed. Renders only when at least one game overlaps your matchup (skipped on an off-day).

### Category Pulse
A summary line above the cards shows your current record and projected end-of-week record, each as a full **W · L · T** (the tie count is always shown, even at `0T`), with a `⚡N close` count between them: `10W · 2L · 0T · ⚡3 close → proj 11W · 1L · 0T`, followed by a **🏆 Win the week: NN%** chip — the 12 per-category win odds collapsed into one number (an exact combinatorial model, not a simulation): the probability you take more categories than your opponent this matchup. 50% is a genuine coin flip; true 100%/0% is rare (12 categories means near-certainty in almost all of them to reach an extreme), so a great week realistically reads ~90%, not 100% — that's expected, not a bug. The same number appears in the inline email body ("This matchup" line) and on the dashboard's Category Pulse tile, so it can't disagree across surfaces.
...
### My Relief Pitchers
Your rostered relievers, showing season SV+H / K / W (from ESPN) plus ERA/WHIP from the best available dataset, a **Recent Form** column, and a role-aware **Score** badge. RP scoring is **skill-weighted (punt-saves)** — see [Composite Scores](#composite-scores). Tapping the Score badge now shows a **season | last-30 recent score** just like starters and hitters — the recent number is built from the reliever's last-30 *rates* (K/IP, SV+H per appearance, ERA, WHIP) paced to a comparable scale, so a hot or cold stretch is visible without changing the headline number.

### Pitcher Recent Form
Your rostered pitchers (SP and RP) sorted hottest → coldest by **Recent Form** — a season-vs-recent composite **Score** delta, not a raw-stat delta, using whichever window is freshest per player (30/15/7-day, tagged next to the value e.g. "15d"). SP compares on `pitcher_score`; RP compares on the rate-paced `rp_score_recent` so a reliever's role-volume season score isn't distorted. Includes a **Whiff%** column (raw swing-and-miss rate, green ≥ 30%) and a role-aware **Score** badge — driven by the exact same comparison as the tap-to-expand dropdown, so a player can't read hot here and cold there. Pitcher names can carry a **$ / ▼ / ▽** buy-low / sell-high chip (the pitcher version of the hitter regression badge): **$** = ERA running *above* his Statcast expected ERA (xERA) → unlucky, buy-low; **▼**/**▽** = ERA *below* xERA → lucky, sell-high (hollow **▽** is the only state a pitcher shows now — see below for why). It's measured relative to the league's typical xERA-vs-ERA offset, and it's **distinct from the ⚠ blowup-risk flag** — ▼/▽ is mean regression (a lucky ERA), ⚠ is single-start disaster risk. Also appears in My Upcoming Starts, My Relief Pitchers, the FA pitcher lists, and Positional Breakdown, and it powers the buy-low / sell-high timing in Trade Radar.

Starters can also carry a **↑ / ↓ bounce-back chip** — green **↑** or red **↓**, distinct from $/▼/▽. This replaced the pitcher-side recent-games confirmation arrow (below) after a walk-forward backtest found that mechanism's direction was backwards for pitchers: a rough recent stretch does **not** predict a continued slide, it predicts a bounce-back, and a hot recent stretch does **not** predict continued excellence, it predicts a letdown. So **↑ bounce-back** = his last few outings were ugly, but his recent FIP (strikeouts/walks/homers only, no batted-ball luck) says the *next* start tends to be better, not worse — don't bench or sell him off the recent line alone. **↓ regression** = he's been running hot, but the same read says the next start tends to cool off — don't chase the streak. Single-start confidence read only, separate from the season $/▼/▽ trade-value badge and from ⚠; trade cards don't show it.

### Hitter Recent Form
Your rostered **hitters** sorted hottest → coldest by **Recent Form** — the same season-vs-recent composite Score delta described above, using whichever window is freshest per player. Includes an **HR%** column (modeled per-game home-run probability, hover for drivers — also listed inside the expanded Score panel for touch devices) and a **Score** badge. Tapping the Score badge shows a breakdown that opens with the **season and recent scores** (each color-coded, with a 🔥/↑/➖/↓/❄ form marker), a compact **Last 30 days** line naming the window stats behind the recent number (e.g. "1.028 OPS · .275 AVG · 8 HR"), and — for a player whose recent form is actually hot or cold — a **Regressed** line judging whether that streak is believable: his recent AVG, weighted down for a small at-bat sample, still compared against his season expected average (xBA). A big enough gap reads "decline/hot stretch looks partly real, not just small-sample variance"; a small one reads "mostly small-sample noise — season skill level still holds up." Then a one-line **scouting read** of the player's archetype, then the drivers behind the number — the exact same comparison as the visible Recent Form column, so a bat can't be 🔥 here yet read "cold" there.
...
- 🔴 **▽** — sell-high, *predicted*: over-performing his expected stats (regression risk — don't chase yet)
- 🔴 **▼** — the same sell-high call, but **confirmed**: his recent games are already trending that way, not just a season-level forecast

The shape carries the meaning on purpose — hollow reads as "watch out," solid reads as "already happening." **This recent-games confirmation is hitters-only now** — the pitcher-side check (comparing recent ERA against xERA) was found backwards by a walk-forward backtest and disabled; see the ↑/↓ bounce-back chip above for the validated pitcher replacement, which reads the opposite direction on purpose.

A small diagonal arrow (**$↗** / **▼↘**) is what actually flips a **hitter's** glyph from hollow to solid — it means the call isn't just a season-level prediction, it's already showing up in her recent games (recent AVG/SLG, weighted for small sample size, still diverging from the season expected stats in the same direction). No arrow doesn't mean the call is wrong, just that there's no recent confirmation yet either way; it's meant to be rare, a stronger version of the same $/▼/▽ signal rather than a separate one. If her recent games are trending the *opposite* way instead, the badge stays as shown but the hover text flags the contradiction.

A **bare arrow with no $/▼/▽** (just 🟢**↗** / 🔴**↘**) means her season totals haven't moved enough yet to trip the full buy-low/sell-high badge, but her recent games are already trending that direction against her own expected stats — an early read that can graduate to a full $/▼ once the season numbers catch up. Hitters only.

These also appear on FA Hitters, the Positional Breakdown, and across the dashboard (Hitting, FA Radar, Weakest Spots). **Tapping the Score badge** explains each chip the player earned, with the exact stat that triggered it (e.g. "SB — 26 SB, top 1% of the league · 28.5 ft/s sprint"). The pitcher Score dropdown does the same for the **QS / 5K+ / 2 / ⚠ / ↑↓** badges (e.g. "QS — projected 6.1 IP · 3 ER is a quality start"; "5K+ — projected 7 strikeouts, backed by a 30% whiff rate"; "⚠ — low floor, blowup-prone: 1.48 WHIP · 5.18 eff. ERA · 6.94 L15 ERA (cold)"; "↑ — recent FIP 5.20 vs season 3.80 xERA, backtesting shows starts like this tend to bounce back"). The 5K+, ⚠, and ↑/↓ badges' hover tooltips carry those same stats.
- 🔥 = Score up 12+ points recent vs season
- ↑ = Score up 5 to 12 points
- ↓ = Score down 5 to 12 points
- ❄ = Score down 12+ points
...
Every score pill in the digest is **tap-to-expand**: click it for a plain-English breakdown of what drove the score, and **click it again to collapse** (no need to find the ✕). If the player is hurt, the breakdown ends with a 🩹 **Injury** line naming which side of the IL he's on (10-day / 15-day / 60-day / OUT / day-to-day), the body part and specifics, and his expected return — so you can weigh a start-or-sit or a trade without leaving the panel. It shows on every score pill, including the trade cards and the Trade Lab.
...
### Weekly Game Plan
Sits at the top of the Transactions band — a coach's read that turns the win-probability model into a plan instead of a report.

**Contest / Concede strip.** Every roto category lands in one of three buckets, from the same calibrated win-odds behind the 🏆 chip above plus a *swing-leverage* read (how much winning-the-week odds move if that one category alone flips from a sure win to a sure loss):
- **🔒 Locked** — win odds ≥ 85%: already yours, no action needed.
- **⚔️ Contest** — a real toss-up whose leverage clears a minimum bar: genuinely worth spending a move on, ranked most-decisive first.
- **✋ Concede** — win odds ≤ 15%, or a toss-up that barely moves the week either way: not worth chasing.

**Up to 4 ranked moves, in two columns.** Candidates are drawn *only* from the same FA Starting Pitcher / Relief Pitcher / Hitter pools shown below — the plan can never recommend a player those tables don't also list. The FA pool is split into a **Hitters** column and a **Pitchers** column (starters + relievers combined), each independently ranked by how much its picks lift your **overall win-the-week odds**, not just a single category — up to 2 cards per side, side by side, so the section reads at a glance instead of scrolling through a single long list. Each card shows the add (with his projected line and start day for a pitcher, or his season contribution for a hitter/reliever), the drop it costs (or a free roster spot), and the before→after odds for both the category and the whole week. A tag in the top-right corner of the card reads **hold** when the add is a durable season-quality upgrade over your starter at that spot, or **streamer** for a short-term volume/form play. A column with no positive-lift candidate shows its own "no moves clear the bar" note rather than being force-filled.

**A move never costs a real rostered asset — but it CAN name one who's now clearly outclassed.** A drop candidate qualifies either of two ways: he's genuinely **fringe** (a *season* score — discounted further for a confirmed, regression-tested slump, the same continuous signal that discounts trade value — below a streamer-tier floor, not the recency-blended score a single bad-but-unconfirmed week can crater), OR he's **outclassed at the exact spot the add fills** (the incoming player beats his season-blended score by a real margin at a shared position — so a mediocre-but-not-technically-fringe starter can still be the honest cost of a clearly better same-position pickup). Either way, he can never be an active closer/setup arm (RP scoring deliberately discounts saves/holds, so a real one can misleadingly look like the "worst" player) or one of your named protected players (a short personal list of true keepers you'd never actually move, regardless of what the value math says). When no drop qualifies either way for an otherwise-good add, the card still shows — ranked purely by how much it lifts your win-the-week odds — with an advisory note in place of a specific drop rather than disappearing outright. The "🔍 No moves clear the bar" note now only appears when there's genuinely no positive-lift candidate at all, not just when the roster has no fringe body to cut. The same two-path logic (and the outclassed-vs-fringe `[outclassed]`/`[surplus]` tag) also governs the drop suggested by the Week-at-a-Glance bullets above.

On the final day of a matchup, move cards are suppressed (no add can swing today's closing matchup) but the win-the-week odds and the Contest/Concede read still show.

**A 📋 clipboard badge cross-references every recommendation.** Any player named as an Add or Drop here or in the Week-at-a-Glance bullets carries a small tan clipboard chip wherever else he's listed in the digest — FA Starting Pitchers/Relief Pitchers/Hitters, My Upcoming Starts, My Relief Pitchers, Hitter Recent Form, Pitcher Recent Form — so spotting him later reminds you he's already part of a suggestion without scrolling back up. Hover (or tap) for the specific reason(s); it's always the last badge on the row and never changes a score or ranking.

### FA Pickup — Starting Pitchers
Free agent starters with a confirmed or projected upcoming start, grouped by date with day headers. Sorted by composite SP score within each day. Starts past Sunday get a `NEXT WK` badge; a pitcher with ≥ 2 starts in the matchup week gets a blue `2` chip. **Only starters with an SP score of 35 or higher are shown** — streamer-tier arms below that are filtered out (tunable via `_FA_SP_MIN_SCORE`). **Each day is capped to the top ~3 arms, reduced by how many starts you already have that day** (thinness-aware): a day you're already covered on shrinks toward its best arms — but **always shows at least 2 pickups when the day has them** (never a lone arm) — while a thin day shows the full three, with a `(+N more)` note when arms are hidden. Since you only get a handful of moves a week, this surfaces depth only where the schedule actually leaves you short. Each starter also gets a **Cats** column (his top season-strength categories, in neutral silver). When a pickup would actually **move a contested category** this matchup, that category **leads the Cats cell as a highlighted `↑` marker** (green when the add would put you over 50%, blue if not) — the category his remaining production most helps, from the same calibrated win-probability model behind the Category Pulse cards. It only shows for a category you're not already winning comfortably, so it answers "does adding him actually flip something?" at a glance; **tap the Score pill to reveal the full *before→after* odds** (e.g. *lifts your K win odds 46% → 58% this matchup*). **The same highlight appears in the Cats column on the FA Relief Pitchers (SVHD) and FA Hitters (R/HR/RBI/SB) tables**, so you can spot the pickup that swings a category across every role. **On the final day of a matchup**, the Transactions band opens with a "⏳ Matchup ends today — only today's games are left" note: with just one day of games remaining, the win-% chips and remaining-production projections go quiet because the *expected* one-day swing is small. It's an explanation for the empty chips, not a claim that a pickup is useless — the league runs daily lineups, so a last-day add who plays before his game locks still counts, and a big single-day line can tip a close category (just unlikely).

**Columns:** Pitcher · **Proj. Line** · Matchup (with opponent OPS on a second line) · **QS%** (larger text; with xERA on a second line under ERA) · ERA · K% (with raw whiff% on a second line) · **Cats** · **Recent Form** (season-vs-recent Score delta, colored) · a small calendar-tagged column naming which window backed it (30d/15d/7d) · Score
...
Top available relievers (must have ≥ 1 SV+H on the season), ranked by RP score (SV+H · K · W · ERA · WHIP — skill-weighted, see [Composite Scores](#composite-scores)). A **Recent Form** column (same season-vs-recent Score comparison as everywhere else, via the rate-paced recent RP read) sits alongside a **Cats** column listing up to 3 roto categories the reliever is strong in (neutral silver), with the one category his pickup most swings this matchup highlighted with an `↑` (full odds in the Score dropdown). Includes a **Save-Role Watch** callout flagging emerging FA closers and fading rostered closers.

### FA Pickup — Hitters
Top available hitters **grouped by position** (C/1B/2B/3B/SS/OF) instead of one blanket top-12 — each position shows 1-3 free agents sized to how much of a need that spot is for you (bottom-third league rank → 3, top-third → 1, middle → 2), with a banner naming your league rank and strength there (`Strong` / `Average` / `Need Help`). A multi-position-eligible free agent (e.g. 2B/SS) is only ever listed once, under whichever position needs him most. On a **Need Help** banner, a "You have:" line lists your current players at that spot with their season\|30-day score and the same Recent Form emoji and **PWR / SB / IL / $ / ▼ / ▽** tactical badges shown elsewhere on the row — so you can judge the upgrade without scrolling back to Positional Breakdown. Columns: R · HR · RBI · SB · OPS · **Recent Form** · **Cats** (up to 3 strong roto categories in silver, with the one his pickup most swings this matchup highlighted `↑`; full odds in the Score dropdown) · **HR%** (modeled per-game HR probability) · Score (see [Hitter Recent Form](#hitter-recent-form)).

### Trade Radar
Trade ideas with rival teams — the one lever the digest can act on beyond your own roster and the free-agent pool. Each card **fixes a rival's category need** (their reason to accept the deal) while **tilting value to you**. You send a player strong in a category you're deep in and the rival is weak in, and get back one who fills a category **or a thin roster position** you need. Only players at a position where you have **surplus** are offered (so no trade ever opens a hole), and your **elite bats are protected** — the radar won't put a genuine masher on the block unless he's a sell-high regression candidate you'd want to move anyway. A short personal list of **named untouchable players** — true keepers you'd never actually move, regardless of value — is hard-excluded from every trade/Blockbuster suggestion too, on top of that value-based protection.

Value is judged on **true category contribution**, not the role-score badge: a closer and an everyday hitter can both post a 90+ score, but the hitter contributes to five categories every day while the closer touches one *punt* category (SV+H) plus a sliver of ERA/WHIP/K in ~60 innings — so an everyday smasher won't be offered straight up for a reliever (saves are discounted since we punt them). Because the hitting game is filling roster holes across C/1B/2B/SS/OF, the radar also surfaces hitters who **upgrade a thin position** even when that position isn't a bottom-third *category*. Hitter value is also adjusted for **positional scarcity** — an everyday bat at a shallow position (a competent catcher) is worth more than an equally productive one at a deep position (first base), so the radar won't tell you a package of outfield depth fairly buys a scarce-position starter. A hitter's value also gets a small nudge from the same recency read behind the **$↗ / ▼↘** confirmation arrow above — a confirmed slump trades for a bit less, a confirmed hot streak for a bit more, and it's symmetric (a rival's slumping player is discounted for you the same way yours would be for them). This keeps the values realistic instead of handing you deals a rival would reject on sight.
...
The radar blends two lanes when building candidates — a **fair** lane (near-even value, or you pay up a touch, the kind that genuinely fetches a scarce upgrade at catcher or shortstop) and a **value** lane (tilts to you, since the rival is still accepting on a genuine category need) — but **only ever shows you a deal a rival would realistically accept.** Every candidate is graded from the *other* manager's point of view (their own need-weighted value, plus a graduated reluctance to part with a star at par), and anything that reads as an "aggressive ask" is dropped before it can ever appear as a card — not shown with a warning chip, simply never suggested. Some days that means a rival gets no card at all; that's the radar being honest about what wouldn't actually land, not a bug. (If you want to see the milder ideas the radar deliberately holds back — realistic asks that just don't excite the rival as much — that's what the Trade Lab's [Partner Fit board](#interactive-trade-lab) is for — it grades every rival, tier by tier. It won't hand you a star-reach either, though: those never surface as a target on either surface, just a "no clean deal" diagnosis.) Where possible, deals also exploit **buy-low / sell-high** timing from Statcast expected-vs-actual stats: move a bat whose surface numbers are about to regress, acquire one due to rebound. Chips flag what's in play — blue = category gained, lime = a thin position filled, and the same **$** (buy-low) / **▼**-or-**▽** (sell-high, solid once confirmed by his recent games, hollow while still just a prediction) glyphs used everywhere else in the digest; the footer tag tells you which way the timing helps you. The card header's realism chip (**realistic** / **aggressive ask**) also carries a trailing **estimated likelihood %** — a heuristic read of how likely the rival is to accept, so two "realistic" cards can still be told apart at a glance. Each player also carries the **same tactical skill badges as the rest of the digest** (PWR/SB for hitters, cyan **QS** / yellow **K+** season-skill + orange **⚠** blowup-risk for starters), so you can read a player's durable strengths right on the card. Every player's **Score pill is tappable** (on both sides of the deal) for the same prose breakdown you get in the section tables. Both 1-for-1 and 2-for-2 shapes appear; the section only shows when a real, realistic candidate exists.
...
**Columns:** Pitcher · **Proj. Line** · Matchup (with opponent OPS on a second line) · **QS%** (larger text; with xERA on a second line under ERA) · ERA · K% (with raw whiff% on a second line) · **Recent Form** (season-vs-recent Score delta, colored) · a small calendar-tagged column naming which window backed it (30d/15d/7d) · Score
...
### Hitter Recent Form is empty
The section needs at least one populated recent-window row (30/15/7-day FantasyPros or the `pybaseball.batting_stats_range` fallback) for your rostered hitters. It's silently skipped only if all of those are empty — most commonly a pybaseball/FanGraphs fetch failure on top of thin FP recent data. Run `python fetch_data.py` locally and look for `Recent hitter stats FAILED`.
...
### Adjusting Recent Form thresholds
Every Recent Form surface (columns, the two Recent Form sections, the Roster KPI, both dashboard tiles) shares ONE pair of thresholds in `fantasy/analytics.py`, on a composite-Score-point scale (not a raw stat scale):
- `_FORM_HOT_DELTA = 12` — recent score − season score ≥ this → 🔥 hot (mirrored: ≤ −12 → ❄ cold)
- `_FORM_WARM_DELTA = 5` — ≥ this (below hot) → ↑ warm (mirrored: ≤ −5 → ↓ cool)

Change these two constants to retune every surface at once — there's no longer a per-table threshold to hunt down.
...
**Starting-pitcher score (`pitcher_score` / `_score_p`):** K% (blended 60/40 with Baseball Savant whiff percentile) + run prevention (ERA blended 55/45 with Savant xERA) + WHIP + contact-quality allowed (barrel%/xwOBA-against) + **quality-start probability** (the modeled `qs_probability`) + **wins**. QS/W replaced the old flat start-volume bonus so a durable, QS-reliable innings-eater gets real credit for a scored roto category, not just a "makes starts" checkmark. Small samples damped toward the mean (role/window-relative innings floor). Displayed blended 65% season / 35% recent form.

**Relief-pitcher score (`rp_score`):** Skill-weighted **punt-saves** build — K, ERA (blended with xERA) and WHIP carry most of the weight; **SV+H is deliberately de-emphasized (~15%)** since it's the most volatile category and one we're willing to sacrifice. A dominant setup man can outrank a mediocre closer. Counting stats prefer ESPN season totals. Small samples damped toward the mean, same as the starting-pitcher score, so a reliever with only a handful of innings can't max out on a fluky ratio.
...
**QS Probability:** Formula-based estimate (no MLB API support). Inputs: IP/G, ERA, WHIP, Brl%, K%, opponent OPS. Baseline = 38% (league average). Key driver is IP/G — uses total games (not just starts) so relief appearances bleed down the innings-depth signal for mixed-role pitchers. Calibration: ace (~75%), league avg (~38%), short reliever making a spot start (~15%). Shown as a color-coded percentage in FA SP and My Upcoming Starts tables: green ≥ 60%, white ≥ 40%, muted < 40%. Also feeds the starting-pitcher score's QS component and trade value's QS credit above.

> **All three role scores self-recalibrate.** The SP/RP/hitter p50→50 / p90→80 constants are re-derived from the live data on every run (`compute_score_calibration`), so the 0–100 scale tracks the season without any hand-editing. If a role's qualified pool is too thin (early season), it falls back to the last hand-tuned constants. `recalibrate_scores.py` is now just a manual inspection tool (prints the current live constants for all three roles) and the home of those fallback values — update them there if you materially change a score's component mix.
