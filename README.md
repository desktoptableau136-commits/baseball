...
The shape carries the meaning on purpose — hollow reads as "watch out," solid reads as "already happening." This applies to **both hitters and pitchers** now (pitchers check recent ERA against xERA the same way hitters check recent AVG/SLG against xBA/xSLG).

A small diagonal arrow (**$↗** / **▼↘**) is what actually flips the glyph from hollow to solid — it means the call isn't just a season-level prediction, it's already showing up in his recent games (recent AVG/SLG for a hitter, recent ERA for a pitcher, weighted for small sample size, still diverging from the season expected stats in the same direction). No arrow doesn't mean the call is wrong, just that there's no recent confirmation yet either way; it's meant to be rare, a stronger version of the same $/▼/▽ signal rather than a separate one. If his recent games are trending the *opposite* way instead, the badge stays as shown but the hover text flags the contradiction.

A **bare arrow with no $/▼/▽** (just 🟢**↗** / 🔴**↘**) means his season totals haven't moved enough yet to trip the full buy-low/sell-high badge, but his recent games are already trending that direction against his own expected stats — an early read that can graduate to a full $/▼ once the season numbers catch up. Also shared by hitters and pitchers.
