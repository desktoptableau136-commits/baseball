...
### Trade Radar
Mutually-beneficial trade ideas with rival teams — the one lever the digest can act on beyond your own roster and the free-agent pool. Each card is a swap where **both** sides address a real need: you send a player strong in a category you're deep in and the rival is weak in, and get back one who fills a category **or a thin roster position** you need. Only players at a position where you have **surplus** are offered (so no trade ever opens a hole in your lineup).

Fairness is judged on **true trade value**, not the role-score badge: a closer and an everyday hitter can both post a 90+ score, but the hitter contributes to five categories every day while the closer touches one *punt* category (SV+H) plus a sliver of ERA/WHIP/K in ~60 innings — so the radar won't offer an everyday smasher straight up for a reliever (value is measured by total category contribution, with saves discounted since we punt them). Because the hitting game is filling roster holes across C/1B/2B/SS/OF, the radar also surfaces hitters who **upgrade a thin position** even when that position isn't a bottom-third *category* — the cyan chip on a "you get" player marks the slot they fill.

Cards are ranked by how well the deal addresses your **deepest season-long needs** (category or positional); both 1-for-1 and 2-for-2 shapes appear. The section only shows when real candidates exist — an empty trade market simply omits it.
