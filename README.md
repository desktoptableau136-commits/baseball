...
Two things help you build the deal quickly. Each player carries a **position chip** by their name, and partner players who'd actually help you are flagged with a **🎯 target** marker (they're strong in a category you need, or upgrade one of your thin positions). Below the ledger, a **Deal Coach** panel shows what each side still needs — split into **category needs** and **position needs**, so a stat gap and a roster-slot gap never read as the same thing — plus your leverage (categories *and* positions where you're deep and they're thin). It then lists **clickable suggestions**, ranked by how much a player actually helps the need side (not just raw trade value): players to *get* that fill your needs and pieces to *offer* that fill theirs (click one to add it), each carrying the same badges as everywhere else (injury, PWR/SB, QS/K+, buy-low/sell-high) plus a quick **estimated accept-odds %** and a warning glyph if adding it would leave a roster thin or short at a position, or a star-value flag if it's the kind of piece that usually costs a sweetener. When nothing squarely matches a need, the list pads in a few clearly-marked **pure-value** picks (dashed, `≈`) rather than pretending they're a recommendation. A **Fair · Favor me · Fleece** strategy toggle tunes how the coach curates: sliding toward *Fleece* protects your best players from the offer list, leads with cheaper fillers, and aims for a bigger value edge in your favor. The toggle only shapes the coaching — the Accept/Counter/Decline verdict stays objective. When a deal grades **Counter** because you'd be paying up, the Coach panel itself **names the specific spare partner piece to request** (a clickable name that adds it straight to the *You get* side), mirroring the digest's counter suggestion — and flags it right alongside a note when your build is one piece away from blockbuster territory.
