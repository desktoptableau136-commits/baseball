...
Each card's realism chip answers "would *they* take it?" Right beside the **Build in Trade Lab** link, a quiet one-line hint — `base +N.NN · you +N.NN · them +N.NN` — gives the net give/get value from all three perspectives (the universal category-value currency, then each side re-priced by its own roster needs), so you don't have to click through just to see whether a deal that looks "realistic" is actually a wash once needs are weighed. There's no accept/counter/decline chip here, deliberately: every Trade Radar idea is something *you'd* propose, and you can't "counter" your own offer — so instead of flagging a bad-for-you deal, the radar simply never generates one that would require shipping your best player at even value in the first place. Every card also has a **Build in Trade Lab** link that opens the exact deal, pre-loaded, in the [Interactive Trade Lab](#interactive-trade-lab) — where the full Accept/Counter/Decline read (and the "Would they/you do it?" pair) does apply, since there you can freely reshape the deal in either direction.
