...
**A move never costs a real rostered asset.** The drop candidate must have a *season* score below a streamer-tier floor — not the recency-blended score a single bad week can crater — and can never be an active closer/setup arm (RP scoring deliberately discounts saves/holds, so a real one can misleadingly look like the "worst" player). On a well-built roster this can mean **no card clears the bar at all**, shown as its own "🔍 No moves clear the bar" note — that's correct: no real bench fat to spare reads as no safe moves, not as license to cut a starter for a middling streamer.
