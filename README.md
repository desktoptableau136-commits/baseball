...
15b. **Trade Radar** — mutually-beneficial trade ideas with rival teams, each with a **would-you-accept-it** read and a **Build in Trade Lab** link *(shown only when candidates exist)*
...
And a **both-parties depth floor** keeps deals sane for the *other* manager too: the radar won't propose taking a rival's only catcher (or shortstop, etc.) for a couple of outfielders — a deal no one would accept because it leaves them unable to field the position. Pure category value misses this (a lone catcher's counting stats are thin, so two productive outfielders "out-value" him on paper), so the floor is a separate body-count check: acquiring a team's last starter at a position requires sending one back. It runs both ways — you won't be shown a deal that guts *your* roster either. Deals that survive but leave a team with a starter and no backup at a one-deep position get read honestly ("aggressive ask," with a note that it *leaves them without a backup at C*) rather than labeled realistic. A real incoming offer that would thin *you* out at a position gets nudged from Accept to **Counter** with the same caution.

The radar surfaces two kinds of deal, side by side. **Realistic** deals land near even value (or you pay up a touch) — the kind a rival would actually accept, and the ones that genuinely fetch a scarce upgrade at catcher or shortstop. **Value plays** tilt to you (the rival accepts on category need, so the value can lean your way) — better for your team, but a tougher sell. Each card is tagged from the *other* manager's point of view — a green **realistic** or an amber **aggressive ask** — so you can tell at a glance which offers are worth actually sending and which need a motivated trade partner. Where possible, deals also exploit **buy-low / sell-high** timing from Statcast expected-vs-actual stats: move a bat whose surface numbers are about to regress, acquire one due to rebound. Chips flag what's in play — blue = category gained, cyan = a thin position filled, and the same **$** (buy-low) / **▼** (sell-high) glyphs used everywhere else in the digest; the footer tag tells you which way the timing helps you. Every player's **Score pill is tappable** (on both sides of the deal) for the same prose breakdown you get in the section tables. Both 1-for-1 and 2-for-2 shapes appear; the section only shows when real candidates exist.

Each card's realism chip only answers "would *they* take it?" A second chip right beside it — **you: accept / counter / decline** — answers the other half: would *you*? It's the same read the Trade Lab gives a loaded deal (including the star-reluctance check — a card won't read "realistic" while quietly hiding a "you'd be shipping your best player at even value" catch that only showed up once you clicked through), with a one-line reason underneath. Every card also has a **Build in Trade Lab** link that opens the exact deal, pre-loaded, in the [Interactive Trade Lab](#interactive-trade-lab) so you can tweak it before sending.
