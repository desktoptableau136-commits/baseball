...
**A move never costs a real rostered asset.** The drop candidate must have a *season* score below a streamer-tier floor — not the recency-blended score a single bad week can crater — can never be an active closer/setup arm (RP scoring deliberately discounts saves/holds, so a real one can misleadingly look like the "worst" player), and can never be one of your named protected players (a short personal list of true keepers you'd never actually move, regardless of what the value math says). When no safe drop exists for an otherwise-good add, the card still shows — ranked purely by how much it lifts your win-the-week odds — with an advisory note in place of a specific drop rather than disappearing outright. The "🔍 No moves clear the bar" note now only appears when there's genuinely no positive-lift candidate at all, not just when the roster has no fringe body to cut.
